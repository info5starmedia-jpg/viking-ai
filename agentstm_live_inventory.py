...
async def get_live_seatmap(event_id: str) -> Dict[str, Any]:
    ...
    if not event_url:
        return {"error": "no_event_url"}

    seatmap = await _scrape_seatmap_via_browserless(event_url)
    if seatmap is None:
        return {"error": "seatmap_scrape_failed", "event_url": event_url}
...
async def summarize_inventory(seatmap: Dict[str, Any]) -> Dict[str, Any]:
    ...
    - total seats
    - total available
    - sections with heat-like stats
    """
    sections: List[Dict[str, Any]] = []
    total = 0
    available = 0

    # Structure of TM seatmap varies; we treat it generically.
